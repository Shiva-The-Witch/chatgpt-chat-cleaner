(() => {
  ...
  shadow.innerHTML = `
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .modal-backdrop {
        ...
        color:var(--text); direction:ltr; font-family:inherit;
      }
      .modal {
      ...
      .preview { max-height:180px; overflow:auto; margin:12px 0; padding:10px 28px 10px 10px; border:1px solid var(--border); border-radius:10px; color:var(--muted); font-size:11px; line-height:1.8; background:var(--surface); }
    ...
    <section class="shell" aria-label="ChatGPT Chat Cleaner & Organizer">
      <div class="panel">
        <header class="header">
          ...
          <div class="heading"><h1>Chat Cleaner</h1></div>
          <div class="header-actions">
            <button class="theme-toggle icon-button ghost" type="button" aria-label="Toggle theme" title="Toggle theme">☾</button>
            <button class="close icon-button ghost" type="button" aria-label="Close" title="Close">×</button>
          </div>
        </header>

        <div class="controls">
          <div class="toolbar">
            <input class="search" type="search" placeholder="Search conversation titles…" />
            <select class="scope" aria-label="Conversation scope">
              <option value="all">All conversations</option>
              <option value="active">Active only</option>
              <option value="archived">Archived only</option>
            </select>
            <input class="older-than" type="date" aria-label="Older than date" title="Show conversations older than this date" />
            <select class="article-filter" aria-label="Content type">
              <option value="all">All</option>
              <option value="articles">Articles only</option>
              <option value="unmarked">Unmarked</option>
            </select>
            <select class="project-filter" aria-label="Project">
          ...
          <div class="actions">
            <div class="action-group">
              <button class="scan primary" type="button">Scan</button>
              <button class="manage-projects" type="button">Projects</button>
              <button class="select-filtered" type="button" disabled>Select filtered</button>
              <button class="select-generic" type="button" disabled>Generic titles</button>
              <button class="select-duplicates" type="button" disabled>Older duplicates</button>
            </div>
            <div class="action-group secondary">
              <button class="clear-selection" type="button" disabled>Clear selection</button>
              <button class="export-catalog" type="button" disabled>Export catalog</button>
              <button class="import-selection" type="button" disabled>Import selection</button>
        ...
        <div class="content">
          <div class="summary">
            <span>Found: <strong class="total">0</strong></span>
            <span>Visible: <strong class="visible">0</strong></span>
            <span>Selected: <strong class="selected-top">0</strong></span>
            <span class="status" role="status" aria-live="polite"></span>
          </div>
          <div class="list"><div class="empty">No conversations loaded.</div></div>
        </div>

        <footer class="footer">
          <span class="count"><strong class="selected-bottom">0</strong> selected</span>
          <span class="spacer"></span>
          <select class="bulk-project" aria-label="Project for selected conversations" disabled>
            <option value="">Move selected to…</option>
          </select>
          <button class="bulk-assign" type="button" disabled>Assign project</button>
          <button class="export-selection" type="button" disabled>Export selection</button>
          <button class="delete danger" type="button" disabled>Delete selected</button>
        </footer>
      </div>
    </section>
  `;
  ...
  function applyTheme(theme) {
    ...
    ui.themeToggle.setAttribute("aria-label", value === "dark" ? "Switch to light mode" : "Switch to dark mode");
    ui.themeToggle.title = value === "dark" ? "Light mode" : "Dark mode";
  }
  ...
  async function scanAll() {
    ...
    try {
      ...
      await getHeaders();
      const active = await fetchConversationPages(false, "active conversations");

      let archived = [];
      try {
        archived = await fetchConversationPages(true, "archived conversations");
      } catch (error) {
        if (error?.status === 401 || error?.status === 403) throw error;
        setStatus("Archived conversations are unavailable.");
      }
      ...
      setStatus(`${formatNumber(state.conversations.length)} conversations found.`);
      render();
    } catch (error) {
  ...
  async function fetchConversationPages(isArchived, label) {
    ...
    while (offset < expectedTotal && page < 200) {
      setStatus(`Loading ${label}… ${formatNumber(results.length)}`);
  ...
  function render() {
    ...
    if (!visible.length) {
      ...
      empty.textContent = hasData ? "No conversations match the current filters." : "No conversations loaded.";
  ...
  function createRow(conversation) {
    ...
    articleButton.textContent = metaData.article ? "Article ✓" : "Article";
    articleButton.title = metaData.article ? "Remove article mark" : "Mark as article";
    ...
    meta.className = "meta";
    meta.appendChild(createBadge(`Updated: ${formatDate(conversation.updateMs)}`));
    if (conversation.isArchived) meta.appendChild(createBadge("Archived", "archived"));
    if (protectedChat) meta.appendChild(createBadge("Current", "protect"));
    if (metaData.article) meta.appendChild(createBadge("Article", "article"));
    ...
    projectPicker.setAttribute("aria-label", `Project for ${conversation.title}`);
  ...
  function selectFiltered() {
    ...
    render();
    setStatus("Filtered conversations selected.");
  }
  ...
  function selectGenericTitles() {
    ...
    render();
    setStatus(`${formatNumber(added)} generic-title conversations selected.`);
  }
  ...
  function selectOlderDuplicates() {
    ...
    render();
    setStatus(`${formatNumber(added)} older duplicates selected.`);
  }
    ...
    try {
      ...
      const ids = Array.isArray(payload?.deleteIds)
        ...
        : Array.isArray(payload?.ids)
          ? payload.ids
          : [];
      if (!ids.length) throw new Error("The selected file does not contain deleteIds.");
      ...
      render();
      setStatus(`${formatNumber(imported)} selections imported${skipped ? `; ${formatNumber(skipped)} skipped` : ""}.`);
    } catch (error) {
      setStatus(`Invalid selection file: ${error?.message || "Unknown error"}`, true);
    }
  }
  ...
  function openDeleteConfirmation() {
    ...
    title.textContent = `Delete ${formatNumber(selectedItems.length)} conversations`;
    const warning = document.createElement("p");
    warning.textContent = "Deletion cannot be undone. Type DELETE PERMANENTLY to confirm.";
    ...
    if (selectedItems.length > 20) {
      const item = document.createElement("li");
      item.textContent = `… and ${formatNumber(selectedItems.length - 20)} more conversations`;
      preview.appendChild(item);
    }

    const phrase = document.createElement("input");
    phrase.type = "text";
    phrase.placeholder = "DELETE PERMANENTLY";
    ...
    phrase.addEventListener("input", () => {
      confirm.disabled = phrase.value.trim() !== "DELETE PERMANENTLY";
    });
  ...
  async function runDeletion(items, card, cancelButton, confirmButton, phraseInput) {
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      progressText.textContent = `Deleting ${formatNumber(index + 1)} of ${formatNumber(items.length)}: ${conversation.title}`;
    ...
    progressText.textContent = fatalFailure === 401 || fatalFailure === 403
      ? `Your session expired. ${formatNumber(deleted)} conversations were deleted before the operation stopped.`
      : fatalFailure === 404 || fatalFailure === 405
        ? `The ChatGPT deletion endpoint appears to have changed. ${formatNumber(deleted)} conversations were deleted before the operation stopped.`
      : `${formatNumber(deleted)} conversations deleted${failures.length ? `; ${formatNumber(failures.length)} failed` : ""}.`;
  ...
  function saveOrganizer() {
    try {
      ...
      setStatus("Unable to save local organization data.", true);
    }
  }
  ...
  function refreshProjectControls() {
    ...
    placeholder.value = "";
    placeholder.textContent = "Move selected to…";
  ...
  function openProjectManager() {
    ...
    card.append(title, newProject, list, actions);
    modal.appendChild(card);
    ui.shell.appendChild(modal);
    draw();
    input.focus();
  ...
  function assignSelectedToProject() {
    ...
    saveOrganizer();
    render();
    setStatus(`${formatNumber(state.selected.size)} conversations assigned to the project.`);
  }
  ...
  function explainApiError(error) {
    if (error?.status === 401 || error?.status === 403) return "Your ChatGPT session could not be detected. Refresh the page and try again.";
    if (error?.status === 429) return "ChatGPT is rate-limiting requests. Try again later.";
    if (error?.status === 404 || error?.status === 405) return "The ChatGPT web interface appears to have changed. The operation was stopped.";
    return `Unable to load conversations${error?.message ? `: ${error.message}` : ""}.`;
  }

  function formatDate(milliseconds) {
    if (!milliseconds) return "Unknown";
    try {
      return new Intl.DateTimeFormat("en-US", { year: "numeric", month: "short", day: "numeric" }).format(new Date(milliseconds));
    } catch {
